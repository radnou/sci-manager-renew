<script lang="ts">
	...
	let selectedLoyer = $derived(loyers.find((loyer) => String(loyer.id || '') === selectedLoyerId) || null);
	let selectedBien = $derived(
		selectedLoyer ? biens.find((bien) => String(bien.id || '') === String(selectedLoyer.id_bien || '')) || null : null
	);

	function buildLoyerOptionLabel(loyer: Loyer) {
		const bien = biens.find((entry) => String(entry.id || '') === String(loyer.id_bien || ''));
		const bienLabel = bien ? (bien.ville ? `${bien.adresse} • ${bien.ville}` : bien.adresse) : 'Bien non identifié';
...
<Card class="sci-section-card">
	<CardHeader>
		...
		<CardContent class="space-y-3 pt-0">
			{#if loyers.length === 0}
				<div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
			...
			{:else}
				<div class="grid gap-3">
					<label class="sci-field" for="quitus-loyer">
						...
						<select id="quitus-loyer" name="quitus-loyer" class="sci-select" bind:value={selectedLoyerId}>
					...
					{#if selectedBien}
						<div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
							<p class="font-semibold">{selectedBien.adresse}</p>
							<p class="mt-1 text-slate-500 dark:text-slate-400">
								{selectedBien.ville || 'Ville non renseignée'} • {selectedBien.type_locatif || 'Type à compléter'}
							</p>
						</div>
					{/if}
			...
			{#if pdfUrl}
				<div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-900">
					<div>
				...
				>
					<div class="rounded-xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100">
			...
			{:else}
				<div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
					...
					<p class="mt-1 text-sm text-slate-500">Sélectionne un loyer puis génère une quittance prête à être envoyée.</p>
				</div>
			{/if}
		</CardContent>
	</Card>
