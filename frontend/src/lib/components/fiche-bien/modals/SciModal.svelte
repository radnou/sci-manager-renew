...
<CrudModal bind:open title="Créer une SCI" size="wide" submitLabel="Créer la SCI" {loading} onsubmit={handleSubmit}>
  <div class="col-span-2">
    <label for="sci-nom" class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">Nom de la SCI</label>
  ...
  <div>
    <label for="sci-siren" class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">SIREN (optionnel)</label>
    <input id="sci-siren" type="text" bind:value={siren} placeholder="123456789" maxlength="9"
      class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" />
  </div>
  <div>
    <span id="sci-regime-label" class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">Régime fiscal</span>
