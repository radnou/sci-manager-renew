...
<div class="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
	<div class="mb-4 flex items-center justify-between">
		...
		<div class="text-right">
			<div class="text-2xl font-bold text-slate-900 dark:text-slate-100">{data.total_mrr.toFixed(0)}€</div>
	...
	<div class="mb-4 rounded-lg bg-slate-50 px-4 py-3 dark:bg-slate-800">
		...
		<span class="ml-2 text-lg font-semibold text-slate-900 dark:text-slate-100">{data.arpu.toFixed(2)}€</span>
	...
	<div class="mb-4 flex h-8 overflow-hidden rounded-full">
		{#each data.breakdown.filter(p => p.percentage > 0) as plan}
			<div
				...
				title="{planLabels[plan.plan] || plan.plan}: {plan.mrr.toFixed(0)}€ ({plan.percentage.toFixed(1)}%)"
			></div>
		{/each}
	</div>
