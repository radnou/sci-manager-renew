<script lang="ts">
	...
	let { icon: Icon, eyebrow, title, description, actions = [], align = 'center' }: Props = $props();
</script>

<div
	class="rounded-2xl border border-dashed border-border bg-muted/50 px-6 py-10 {align === 'center'
		? 'text-center'
		: 'text-left'}"
>
	{#if eyebrow}
	...
	{#if description}
		<p class="mt-1.5 max-w-sm text-sm text-muted-foreground {align === 'center' ? 'mx-auto' : ''}">
			{description}
		</p>
	{/if}
	{#if actions.length > 0}
		<div class="mt-5 flex flex-wrap gap-3 {align === 'center' ? 'justify-center' : ''}">
			{#each actions as action}
				{#if action.href}
					<Button href={action.href} variant={action.variant ?? 'default'} size="sm"
						>{action.label}</Button
					>
