...
{:else}
	...
	<div
		class="relative"
		onclick={(e) => { e.preventDefault(); e.stopPropagation(); showPrompt = true; trackEvent(EVENTS.DEMO_LOCKED_ACTION, { action }); }}
		onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); showPrompt = true; trackEvent(EVENTS.DEMO_LOCKED_ACTION, { action }); } }}
	>
		{@render children()}
