...
class TestGenerateDeclaration2065:
    ...
    def test_generate_2065_success(self, client, mock_supabase):
        ...
        mock_supabase.table("credits").select().eq().execute.return_value = type("Result", (), {
            "data": [{"montant_mensuel": 500, "duree_mois": 240, "date_debut": "2020-01-01"}]
        })()
...
class TestGetDeclaration2065PDF:
    ...
    def test_get_pdf_not_implemented(self, client):
        """⏳ PDF non implémenté (501)."""
        response = client.get(f"{BASE}/2025/pdf", headers={"x-test-auth": "user-123"})
        assert response.status_code == 501
        assert "développement" in response.json()["detail"]
